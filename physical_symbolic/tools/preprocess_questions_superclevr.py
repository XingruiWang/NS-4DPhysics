...
def main(args):
    ...
    print("Loading part information")
    with open('/home/xingrui/projects/superclevr-physics/physical_symbolic/metadata.json', 'r') as f:
        superclevr_metadata = json.load(f)
    superclevr_metadata['types']['_inv_shapename'] = {v:k for k,v in superclevr_metadata['types']['Shapename'].items()}

    def preprocess_answer_word(answer, _inv_shapename):
        if answer in _inv_shapename:
            return _inv_shapename[answer]
        else:              
            return answer
    ...
    if args.input_vocab_json == '' or args.expand_vocab == 1:
        ...
        if 'answer' in questions[0]:
            answer_token_to_idx = preprocess_utils.build_vocab(
                (preprocess_answer_word(q['answer'], superclevr_metadata['types']['_inv_shapename']) for q in questions)
            )  
    ...
    for orig_idx, q in enumerate(questions):
        ...
        if 'answer' in q:
            answers.append(vocab['answer_token_to_idx'][preprocess_answer_word(str(q['answer']), superclevr_metadata['types']['_inv_shapename'])])
