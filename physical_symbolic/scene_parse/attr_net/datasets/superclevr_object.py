...
class SuperClevrObjectDataset(Dataset):
    ...
    def filter_object(self, type, split):
        ...
        print(split)
        if split == 'train':
            # all_id = all_id[:int(len(all_id) * 0.5 * self.trim)]
            all_id = all_id[:100]
        elif split == 'val':
            # all_id = all_id[int(len(all_id) * 0.5):(int(len(all_id) * 0.5) + int(len(all_id) * 0.1 * self.trim))]
            all_id = all_id[int(len(all_id) * 0.5):int(len(all_id) * 0.5) + 32]
        elif split == 'test':
            # all_id = all_id[int(len(all_id) * 0.6):]
            all_id = all_id[int(len(all_id) * 0.5):int(len(all_id) * 0.5) + 32]                   
