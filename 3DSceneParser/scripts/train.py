...
def train(cfg):
    ...
    model = construct_class_by_name(
        **cfg.model, cfg=cfg, cate=cfg.args.cate, mode='train',
        ...
        mesh_loader=train_dataset.mesh_loader if hasattr(train_dataset, 'mesh_loader') else None,
        checkpoint=cfg.args.resume)

    logging.info("Start training")

    for epo in range(model.start_epoch, cfg.training.total_epochs):
        num_iterations = int(cfg.training.scale_iterations_per_epoch * len(train_dataloader))

        with tqdm(train_dataloader, desc=f"Epoch {epo+1}/{cfg.training.total_epochs}") as t:
            for i, sample in enumerate(t):
                # if i >= num_iterations:
                #     break
                loss_dict = model.train(sample)
                t.set_description(f"Epoch {epo+1}/{cfg.training.total_epochs} | State: {model.get_training_state()}")
            if (epo + 1) % cfg.training.log_interval == 0:
                logging.info(
                    f"[Epoch {epo+1}/{cfg.training.total_epochs}] {model.get_training_state(needReset=True)}"
                )
            ...
            else: # always save last ckpts
                torch.save(model.get_ckpt(epoch=epo+1, cfg=cfg.asdict()), os.path.join(cfg.args.save_dir, "ckpts", f"last_checkpoints.pth"))
