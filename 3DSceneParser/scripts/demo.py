...
def parse_args():
    ...
    parser.add_argument("--vis_inference", action='store_true')
    parser.add_argument("--physics_prior", action='store_true')
    ...
    for cate in all_categories:
        ...
        model = construct_class_by_name(
            **cfg.model,
            ...
            device="cuda:0",
            mesh_loader=val_dataset.mesh_loader if hasattr(val_dataset, 'mesh_loader') else None
        )

        img = cv2.imread("/home/xingrui/projects/superclevr-physics/data/output_patch/frame_00130/patch_3.png", cv2.IMREAD_UNCHANGED)
        H, W, C = img.shape
        # TypeError: img should be Tensor Image.
