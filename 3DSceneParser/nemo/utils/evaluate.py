...
def position_error(gt, pred):

    if pred is None:
        return 1000
    transloation_gt =  np.array(gt["translation"])
    transloation_pred = np.array(pred["translation"])

    # mse
    error_ = (np.square(transloation_gt - transloation_pred)).mean()
