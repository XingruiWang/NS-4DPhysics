...
class MeshLoader():
    def __init__(self, category, base_mesh_path, loader=pascal3D_loader, pad=False, to_torch=False):
    ...
    def get_mesh_para(self, to_torch=False):
        if to_torch:
            if torch.is_tensor(self.all_vertices[0]):
                return self.all_vertices, self.all_faces
            else:
                return [torch.from_numpy(t) for t in self.all_vertices], [torch.from_numpy(t) for t in self.all_faces]
        else:
            if not torch.is_tensor(self.all_vertices[0]):
                return self.all_vertices, self.all_faces
            else:
                return [t.numpy() for t in self.all_vertices], [t.numpy() for t in self.all_faces]
