...
def func_single(meshes, **kwargs):
    # Same mesh for each image in batch
    return meshes, meshes.verts_padded()


def func_reselect(meshes, indexs, **kwargs):
    # Different mesh for each image in batch
...
def func_multi_select(meshes, indexs, transforms, **kwargs):
    # Multi meshes in each image in batch
    ...
    for transform_, index_ in zip(transforms, indexs):
        ...
        verts_ = [transform_[i].transform_points(meshes._verts_list[index_[i]]) for i in range(n_obj) if index_[i] != -1]
        idx_shift = torch.cumsum(torch.Tensor([0] + [t.shape[0] for t in verts_][:-1]), dim=0).to(meshes.device)
        faces_ = [meshes._faces_list[index_[i]] + idx_shift[i] for i in range(n_obj) if index_[i] != -1]
...
class PackedRaster():
    def __init__(self, raster_configs, object_mesh, mesh_mode='single', device='cpu', ):
        ...
        cameras = PerspectiveCameras(focal_length=raster_configs.get('focal_length', 1400) / raster_configs.get('down_rate'), principal_point=((feature_size[1] // 2, feature_size[0] // 2,), ), image_size=(feature_size, ), in_ndc=False, device=device)
...
def get_one_standard(raster, camera, mesh, func_of_mesh=func_single, restrict_to_boundary=True, dist_thr=1e-3, kp_only=False, **kwargs):
    ...
    cam_loc = -torch.matmul(torch.inverse(R.transpose(1, 2)), T[..., None])[:, :, 0]
    ...
    if restrict_to_boundary:
        # (B, K)
        inner_mask = torch.min(camera.image_size.unsqueeze(1) > project_verts, dim=-1)[0] & \
                 torch.min(0 < project_verts, dim=-1)[0]
        ...
        inner_mask = torch.ones(project_verts.shape[:2], dtype=bool, device=project_verts.device)

    if kp_only:
        return project_verts, inner_mask
    ...
    sampled_dist_per_vert = torch.nn.functional.grid_sample(depth_, grid.flip(-1), align_corners=True, mode='nearest')[:, 0, 0, :]
    ...
    # depth_ = depth_.clamp(min=6)
    # iidx = 47
    # tt = (depth_[iidx] - depth_[iidx].min() ) / (depth_[iidx].max() - depth_[iidx].min() )
    # kps = project_verts.cpu().numpy()
    # point_size = 4
    # def foo(t0, kps, vis_mask_):
    #     im = Image.fromarray((t0.cpu().numpy()[0] * 255).astype(np.uint8)).convert('RGB')
    #     imd = ImageDraw.ImageDraw(im)
    #     for k, vv in zip(kps, vis_mask_):
    ...
    # foo(tt, kps[iidx], vis_mask[iidx]).save('debug/tem2.png')
    # import ipdb
    # ipdb.set_trace()
