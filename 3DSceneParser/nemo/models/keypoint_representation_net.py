...
dino_pretrained_url = {
    'vits16': "https://dl.fbaipublicfiles.com/dino/dino_deitsmall16_pretrain/dino_deitsmall16_pretrain.pth",
    'vits8': "https://dl.fbaipublicfiles.com/dino/dino_deitsmall8_pretrain/dino_deitsmall8_pretrain.pth",
    'vitb16': "https://dl.fbaipublicfiles.com/dino/dino_vitbase16_pretrain/dino_vitbase16_pretrain.pth",
    'vitb8': "https://dl.fbaipublicfiles.com/dino/dino_vitbase8_pretrain/dino_vitbase8_pretrain.pth",
}


class ViTs(nn.Module):
    def __init__(self, vv=8, max_layer=9, pretrain=True):
        super().__init__()
        self.net = torch.hub.load('facebookresearch/dino:main', 'dino_vits%d' % vv, )
        if pretrain:
            state_dict = torch.hub.load_state_dict_from_url(
                url=dino_pretrained_url['vits%d' % vv],
                map_location="cpu",
            )
    ...
    def forward(self, x):
        ...
        return x[:, 1:, :].permute(0, 2, 1).contiguous().view(n, -1, h // self.stride, w // self.stride)


def vits8(pretrain=True, vv=8):
    return ViTs(pretrain=pretrain, vv=vv)
