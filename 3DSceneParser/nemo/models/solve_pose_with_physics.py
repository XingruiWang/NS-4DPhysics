...
class SuperClevrProposal():
    ...
    def vis(self, cfg, feature_map, clutter_score, R, T, pred, **kwargs):
        ...
        folder_name = 'kpt_score_map_cont'
    ...
    def gt_proposal(self, cfg, feature_map, clutter_score, R, T, prev_preds, **kwargs):
        ...
        return angles, translation, scales, kwargs['label']
    ...
    **kwargs
    ...
    for i in range(b):            
        if img_name[i].split('/')[-1] == 'rgba_00000.png':
            ...
            prev_frame_name = f'rgba_{str(frame_id-1).zfill(5)}.png'
            prev_pred = prev_pred[video_name][prev_frame_name] # format same as pred['final']
            epochs = 50
        prev_preds.append(prev_pred)
    
    physics_start = time.time()
    init_angles, init_translation, scales, init_obj_idxs = proposal_module.physics_proposal(cfg, feature_map=feature_map, \
                                                                            clutter_score=clutter_score, R=R, T=T, img_name = img_name, prev_preds = prev_preds, **kwargs)
    physics_end = time.time()
    print(f'Physics proposal time: {physics_end - physics_start}')
    # gt_rotation = matrix_to_axis_angle(axis_angle_to_matrix(matrix_to_axis_angle(kwargs['transforms'][..., :3, :3])))
    
    # gt_rotation = kwargs['transforms'][..., :3, :3]
    ...
    # rotation = torch.nn.Parameter(init_rotation.to(device), requires_grad=True)
    ...
    # rotation = euler_angles_to_matrix(angles[:, :, :-1], "XYZ")
    # img_pred = visual_clevr(translation, rotation, init_obj_idxs, R, T, cfg.args.save_dir, distances=angles[:, :, -1], **kwargs)
    
    # for epo in tqdm(range(epochs)):
    for epo in range(epochs):
        # why the loss does not decrease?
        rotation = euler_angles_to_matrix(angles, "XYZ")
        # rotation.retain_grad()
        # distance.retain_grad()

        m_matrix = to_4x4(rotation)
        # trans_ = [Transform3d(matrix=m_matrix[ii]).compose(Scale(scales[ii])).compose(Translate(translation[ii, :, 0])) for ii in range(rotation.shape[0])]
        ...
        #     # Visualize the feature score similarity and the pose estimation
        #     img_pred = visual_clevr(translation, rotation, init_obj_idxs, R, T, cfg.args.save_dir,distances=angles[:, :, -1], **kwargs)
        #     proposal_module.vis(cfg, feature_map=feature_map_vis, clutter_score=clutter_score, R=R, T=T, pred = img_pred, epoch = epo, **kwargs)
        # if epo % 20 == 0:
        #     object_score_vis = object_score[0].detach().cpu().numpy()
        #     clutter_score_vis = clutter_score[0].detach().cpu().numpy()
        #     object_score_vis = object_score_vis / np.max(object_score_vis) * 255
        #     clutter_score_vis = clutter_score_vis / np.max(clutter_score_vis) * 255
        #     score = np.hstack((object_score_vis, clutter_score_vis))
        #     cv2.imwrite(f'object_score.png', score)
        #     img_pred = visual_clevr(translation, rotation, init_obj_idxs, R, T, cfg.args.save_dir,distances=angles[:, :, -1], **kwargs)
        #     cv2.imwrite(f'pred.png', img_pred)
        #     import ipdb; ipdb.set_trace()
            
        #     import ipdb; ipdb.set_trace()
    
    # print('optimized', translation, angles)
    ...
    img_pred = visual_clevr(translation, rotation, init_obj_idxs, R, T, cfg.args.save_dir,distances=scales, **kwargs)
