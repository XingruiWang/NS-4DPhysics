...
class FeatureBankNeMo(nn.Module):
    ...
    def copy_memory(self, memory_ckpt, exact_match=False):
        if not (memory_ckpt.shape[0] == self.memory_pos.shape[0] + self.memory_neg.shape[0]) and exact_match:
            raise ValueError(f"Memory size mismatch: {memory_ckpt.shape[0]} vs {self.memory_pos.shape[0]} + {self.memory_neg.shape[0]}")
        # import ipdb; ipdb.set_trace()
        self.memory_pos.copy_(
            memory_ckpt[0 : self.memory_pos.shape[0]]
        )

        self.memory_neg.copy_(
            memory_ckpt[self.memory_pos.shape[0] : self.memory_pos.shape[0] + self.memory_neg.shape[0]]
        )
